...
server <- function(input, output) {
  ...
  output$leafdown <- renderLeaflet({
    rv_update_leafdown()
    update_leafdown_map(my_leafdown, input, df_ccc_scores, my_leafdown$curr_data)
  })

  observeEvent(input$leafdown_glify_click, {
    click_lat <- input$leafdown_glify_click$lat
    click_lng <- input$leafdown_glify_click$lng
    markers <- avg_temp_per_displ_marker(df_ccc_scores, my_leafdown$curr_data)
  ...
  output$bar_plot <- renderPlot({
    ...
    create_bar_plot(curr_sel_data, curr_map_level, df_ccc_scores, active_markers)
  })
  ...
  output$scatter_plot <- renderPlot({
    ...
    curr_map_level <- my_leafdown$curr_map_level
    active_markers <- rv_active_markers()
    create_scatter_plot(curr_sel_data, curr_map_level, df_ccc_scores, active_markers)
  })
